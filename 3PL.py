...
def neg_log_likelihood(data, theta, beta, alpha, gamma):
    ...
    for i, q in enumerate(user_id):
        ...
        log_lklihood += cij * safe_log(gamma_j + exp) + (1 - cij) * safe_log(1 - gamma_j) - safe_log(1 + exp) # 3.6.1 in the report
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
    return -log_lklihood
...
def update_theta_beta(data, lr, theta, beta, gamma, alpha, update_gamma=False):
...
def irt(data, val_data, lr, iterations, lr2):
    """ Train IRT model.
    ...
    for i in range(iterations):
        ...
        theta, beta, alpha, gamma = update_theta_beta(data, lr, theta, beta, gamma, alpha, update_gamma=False)
    
    for j in range(iterations):
        theta, beta, alpha, gamma = update_theta_beta(data, lr2, theta, beta, gamma, alpha, update_gamma=True)
...
def main():
    ...
    # 0.005, 45 -> Validation accuracy:  0.7082980524978831

    theta, beta, val_acc_lst, train_likelihood, val_likelihood, alpha, gamma = \
        irt(train_data, val_data, lr, iterations, lr2=lr2)
