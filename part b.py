...
def neg_log_likelihood(data, theta, beta, alpha, gamma):
    ...
    for i in range(len(user_id)):
        ...
        log_lklihood += cij * np.log(y) + (1-cij)*np.log(1.0 - np.exp(y))
...
def update_theta_beta(data, lr, theta, beta, alpha, gamma):
    ...
    for i in range(len(user_id)):
        ...
        cij = is_correct[i]
        x = alpha[q]*(theta[u] - beta[q])
        y = gamma[q] + (1 - gamma[q])*(np.exp(x)*sigmoid(x))
        ...
        theta[u] += lr * ((cij*theta1/(gamma[q] + (1-gamma[q])*pij))+ ((1-cij)*-theta1/(1-(gamma[q] + (1-gamma[q]))*pij)))
        # -cij + sigmoid(x) is the gradient with respect to beta
        alpha[q] += lr* ((cij*alpha1/(gamma[q] + (1-gamma[q])*pij)) + ((1-cij)*-alpha1/(1-(gamma[q] + (1-gamma[q])*pij))))
        gamma[q] += lr*(cij*(1-pij)/(gamma[q] + (1-gamma[q])*pij) + ((1-cij)*(1-pij)/(1-(gamma[q] + (1-gamma[q])*pij))))
        beta[q] += lr * ((cij*beta1/(gamma[q] + (1-gamma[q])*pij)) + ((1-cij)*-beta1/(1-(gamma[q] + (1-gamma[q]))*pij)))
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
